...
17 Jan 2021 - Version 2 - Checking for inaccuracies in applied model... 
	...
	- Implemented forced wake options
	- Adjusted Forced Wake (F_w) conditions to be relative to sleep drive (D_v) > 2.4.

8 Apr 2021 - Version 3 - Updated based on email from Dr. Postnova and Dr. Pierson's commentary
	- Corrected Sigmoid function and KSS calculation per Dr. Postnova and Dr. Pierson
...
19-20 June 2021 - Version 5
	- Allowed setting of day for annual calculations, useful if you want to simulate a subset of a year or start/stop at multiple points. 
...
19 May 2022 - Version 6
    - Just cleaned up a little of the internal comments for posting to Github
    - Made sure this works in Python 2.7 and 3.x
...
class subject:
	...
	def t_crit(self, day_history):
		...
		"""
		# This is a little weird, because -2.98 is arbitrary AND it will appear twice in a normally-entrained 24 hour period. Reasonably, I think, I search for when -2.98 occurs only when Y > X."
	...
	def _internal_simulate_day(self, starting_state, I_list, S_list=False ):
		...
		for i_t in range( total_steps ): # i_t being the time index
			...
			if S_list:
				...
				if (day_history[-1].D_v > 2.4 or this_state.get_S(day_history[-1].V_m) < 0.1) and this_S > 0.9:
					this_F_w = 1.0
